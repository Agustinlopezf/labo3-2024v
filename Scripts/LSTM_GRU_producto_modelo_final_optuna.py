...
nombre_experimento = 'LSTM_GRU_producto_modelo_final_optuna'
nombre_carpeta_optuna = 'LSTM_GRU_producto_optuna'
ventana_input = 12
ventana_output = 2
...
def predecir(x, model, scaler):
    ...
    y_pred = y_pred_s.reshape(ventana_output,1) * scaler.scale_[0] + scaler.center_[0] #Tomar los valores del scaler para la columna 1 de tn y antitransformar
...
for producto in ventas_producto_mes['product_id'].unique():
    ...
    if len(ventas_mes_por_producto_features) >= (ventana_input + ventana_output):
        ...
        model.fit(X, Y, epochs=150, batch_size=24, callbacks=[early_stop, reduce_lr], verbose=0)
...
resultados_kaggle_LSTM.to_csv(os.path.join(carpeta_exp, 'predicciones_LSTM-GRU_modelo_final_optuna.csv'), index= False)
